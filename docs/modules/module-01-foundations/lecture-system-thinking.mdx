---
title: Lecture - System Thinking for AI Applications
hide_title: true
sidebar_position: 2
slug: /modules/module-01-foundations/lecture-system-thinking
description: Lecture on system thinking for modern AI applications.
---
...
```mermaid
flowchart TD
    U["User or upstream service"] --> A["Application layer<br/>UI, API contract, permissions, workflow"]
    A --> O["Orchestration layer<br/>prompt assembly, routing, policy checks"]
    O --> R["Retrieval and context layer<br/>search, chunking, ranking, memory lookup"]
    O --> T["Tool layer<br/>APIs, databases, code execution, business actions"]
    O --> M["Model layer<br/>LLM, classifier, ranker, embedder"]
    R --> O
    T --> O
    M --> O
    O --> P["Post-processing and validation<br/>schema checks, citations, safety filters"]
    P --> A
    A --> U
    O -. traces, metrics, eval samples .-> E["Observability and evaluation"]
```

This diagram is intentionally simplified, but it already illustrates a key point: the model is only one node in the
request path.
...
<ResourceLinks
  ...
  items={[
    ...
    {label: 'Module 09 - System Design Patterns', href: '/docs/modules/module-09-system-design-patterns'},
  ]}
/>
